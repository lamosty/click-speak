...
class ClickSpeakApp(rumps.App):
    ...
    def _is_media_playing() -> bool:
        """Check if system media is currently playing via MediaRemote framework."""
        try:
            ...
            fns: dict = {}
            objc.loadBundleFunctions(bundle, fns, [
                ("MRMediaRemoteGetNowPlayingApplicationIsPlaying", b"v@@?", "", {
                    "arguments": {1: {"callable": {
                        "retval": {"type": b"v"},
            ...
            fns["MRMediaRemoteGetNowPlayingApplicationIsPlaying"](queue, callback)
            done.wait(timeout=1.0)
            return result[0]
        except Exception:
            logger.exception("Failed to check media playing state")
            return False
    ...
    def _resolve_configured_device(self, *, refresh: bool = False) -> bool:
        """Resolve configured device name to current PortAudio index."""
        ...
        try:
            ...
            logger.exception("Failed to resolve configured device '%s'", self.config.audio_device)
            self._device_available = False
            return False
    ...
    def _start_listening(self) -> bool:
        if self.config.audio_device:
            # On-demand opens/closes the stream often, so always remap by name
            # to avoid stale Bluetooth device indices after reconnect.
            refresh_before_start = self.config.trigger_mode == "on_demand"
            if not self._resolve_configured_device(refresh=refresh_before_start):
                self._set_status(f"Waiting for {self.config.audio_device}...")
                self._set_icon("idle")
                return False
        ...
        try:
            ...
            # Retry once after forcing a PortAudio refresh. This covers BT
            # reconnects where cached indices changed since the last start.
            if self.config.audio_device and self._resolve_configured_device(refresh=True):
                logger.warning("Audio start failed, retrying once after refresh")
                try:
                    self.audio.start()
                except Exception:
    ...
    def _on_device_reconnected(self) -> None:
        ...
        self._device_available = True
        if self.config.trigger_mode == "on_demand":
            # Keep on-demand mode idle; only mark the device as available.
            self._listening = False
            self.listening_item.title = "Start Listening"
            self.wake_word.enabled = False
            try:
            ...
            self._set_status("Ready")
            self._set_icon("idle")
            logger.info("Device '%s' reconnected (on-demand ready)", self.config.audio_device)
            return
        self._listening = True
        self.listening_item.title = "Stop Listening"
        self.wake_word.enabled = self.config.trigger_mode in ("wake_word", "both")
    ...
    def _handle_hotkey(self) -> None:
        ...
        if self.config.audio_device and not self._device_available:
            if self.config.trigger_mode == "on_demand":
                logger.info("HOTKEY: probing for %s", self.config.audio_device)
                if self._resolve_configured_device(refresh=True):
                    logger.info("HOTKEY: %s is available again", self.config.audio_device)
                else:
